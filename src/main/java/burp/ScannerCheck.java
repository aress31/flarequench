...
public class ScannerCheck implements IScannerCheck {
  ...
  // TODO: Make sure that CrimeFlare is reachable/sends a response back
  // if not print an message.
  private boolean queryCrimeFlare(String host) {
    ...
    try {
      ...
      if (helpers.bytesToString(response).contains("these are not CloudFlare-user nameservers.")) {
        this.burpExtenderCallbacks.printOutput("No entry found within the CloudFlare database");
        return false;
      } else {
        this.burpExtenderCallbacks.printOutput("Existing entry/ies found within the CloudFlare database");
        return true;
      }
  ...
  public List<IScanIssue> doPassiveScan(IHttpRequestResponse baseRequestResponse) {
    ...
    // TODO:
    // 1. Check that the below code behaves as expected.
    // 2. Make sure to send a maximum of 1 request (to CrimeFlare) per host being
    // scanned
    // in order to reduce the total number of requests sent.
